...
// Skips the link if the title doesn't exist (e.g., if it failed to import)
func (s *AgencyStore) LinkAgencyTitle(ctx context.Context, agencyID, titleNumber int) error {
	// Check if the title exists first to avoid foreign key constraint violations
	var exists bool
	checkQuery := `SELECT EXISTS(SELECT 1 FROM titles WHERE title_number = $1)`
	if err := s.db.QueryRowContext(ctx, checkQuery, titleNumber).Scan(&exists); err != nil {
		return fmt.Errorf("failed to check if title %d exists: %w", titleNumber, err)
	}

	if !exists {
		// Title doesn't exist (likely failed to import), skip linking
		return nil
	}
